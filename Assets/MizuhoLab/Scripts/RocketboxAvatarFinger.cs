...
// 関節角度部分を改変
// for Microsoft Rocketbox Avatar Library

using UnityEngine;
using System.Collections.Generic;

public class RocketboxAvatarFinger : MonoBehaviour
{
    ...
    private List<Transform> jointPool = default;
    [SerializeField, TooltipAttribute("各関節が根本の角度に対してどの割合で曲がるかを設定（未指定のときAwakeで自動設定）")]
    private List<float> jointLevelPool = default;
    ...
    void AutoSetupFingerType()
    {
        ...
        string[] typeNames = { "Finger0", "Finger1", "Finger2", "Finger3", "Finger4"};
        int typeIndex = (int)FingerType.L_Thumb;
        //if (name.IndexOf("right") >= 0) { typeIndex = (int)FingerType.R_Thumb; }
        if (name.IndexOf("R") >= 0) { typeIndex = (int)FingerType.R_Thumb; }
    ...
    void Update()
    {
        ...
        // 親指のとき、近接センサーから指が離れている状態をもっと指を開く
        if(isThumb && touchLevel <= 0.05f)
        {
            if(OVRInput.Get(trigger) > triggerStart)
            {
                touchLevel = -0.5f;
            }
            else
    ...
    void SetupFingerType(FingerType type)
    {
        ...
        switch (fingerType)
        {
            case FingerType.L_Thumb:
                ...
                axis = new Vector3(-0.1f, 0.0f, -0.4f);
                maxAngle = 45.0f;
                isThumb = true;
                break;
            ...
            case FingerType.L_Pinky:
                trigger = OVRInput.RawAxis1D.LHandTrigger;
                //axis = new Vector3(0, 1f, -0.1f);
                axis = new Vector3(0, 0.1f, -1.0f);
                break;
